...
class MinibatchMaker(TrainAndTest):
    ...
    def __init__(self, feature_type, flag_gpu, is_validation="train", text_count=0, text_div=0, change_num=0):
        super(MinibatchMaker, self).__init__(
            feature_type, flag_gpu=0, change_num=change_num)
    ...
    def already_dats(self):
        filepath_already = sorted(
            glob.glob(self.OUTPUT_PATH_TEST.format(self.FEATURE_TYPE, "*", "*")))
        already_names = []
        for x in filepath_already:
            # already_names.append(x[-25:-4])
            already_names.append("_".join(x.split("_")[-5:])[:-4])
        return already_names

    def epoch_pickle_test(self, q):
        #filepath = glob.glob(self.INPUT_PATH)
        filepath_part = glob.glob(self.INPUT_PATH_TEST)
        # filepath_part = sorted(self._fetch_data(
        #     glob.glob(self.INPUT_PATH_TEST), self.is_validation))
        file_count = 0
        already_names = self.already_dats()
        self.div_num = 3
        if self.is_validation == "test":
            # with codecs.open("test" + str(self.text_div) + ".dat", 'w', 'utf-8') as f:
            #     f.write(filepath_text)
            # exit()
            not_yet = []
            for file in filepath_part:
                name = file.split("/")[-1][:-7]
                #file_num = int(name.split("_")[-1])
            ...
            # for file in not_yet[int(((self.div_t - 1) * len(not_yet)) //
            # 4):int((self.div_t * len(not_yet)) // 4)]:
            for file in not_yet:
                name = file.split("/")[-1][:-7]
                file_num = int(name.split("_")[0])
                #file_num = int(name.split("_")[-1])
                # if name in already_names:
                if name in already_names:
                    continue
                # if not (((self.div_t - 1) * len(filepath_part)) // 16 < file_num and file_num <= (self.div_t * len(filepath_part)) // 16):
                #     continue
                # koko
                ...
                if self.FEATURE_TYPE == "rnn_att":
                    ...
                    if len(dec_scores) == 0:
                        ...
                        continue
                    # if text_name != text_name_keeper:
                    q.put((name, enc_words, fnn_inputs))
                    # with codecs.open("test" + str(self.text_div) + ".dat", 'w', 'utf-8') as f:
                    #     print(train_info_dict[
                    #           list(train_info_dict.keys())[-1]])
                    #     # f.write(
                    #     #     "\n".join(map(str, list(train_info_dict.keys(
                    # exit()
                    # vec_data_dict_init_text_genre, vec_data_dict_init_text_text_name = self._vec_data_dict_maker(
                    #     train_info_dict)
                    # data_list_x, _ = self._vec_addeder(
                    #     data_list_x, train_info_dict)

                    # q.put((name, data_list_x))
                    # if vec_data_dict_init_text_genre != None:
                    #     vec_data_dict_sub = {}
                    #     vec_data_dict_sub["vec_for_text_dic"] = {}
                    #     vec_data_dict_sub["cf_vec_for_text_dic"] = {}
                    #     self.vec_data_dict[
                    #         vec_data_dict_init_text_genre] = vec_data_dict_sub
                    #     self.vec_data_dict[vec_data_dict_init_text_genre][
                    #         "text_name"] = vec_data_dict_init_text_text_name
    ...
    def _fetch_data(self, filepath, is_validation="train", is_BCCWJ=True):
        ...
        print(valid_dic)
        print(str(self.text_div) + is_validation)
